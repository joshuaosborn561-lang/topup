...
Default (D40): look at the client first. Top up campaigns that are working. One pull per shared ICP; segment into those campaigns. The receipt plus its outcome is the recipe. Load `topup.pull_receipts` for the lane and sibling receipts that share the ICP, `topup.v_receipt_outcome`, `topup.v_lane_runway`, and `topup.lane_exclusions`. Inventory in our own tables is the first card (free). Otherwise a reasoner proposes repeat / widen / new_segment / hold. Code validates. Josh taps the segment before any paid step. A handwritten `recipes/<client>/<lane>.json` still wins. Missing `company_size` is backfilled (D38): cache and other sized leads first, then getleads counts (unlimited, $0), then Wikidata / Clearbit suggest / OpenCorporates (free), then one LeadMagic company-search leftover pass whose spend for the whole backfill is $5. Physical / signal sources park until those adapters are wired.
